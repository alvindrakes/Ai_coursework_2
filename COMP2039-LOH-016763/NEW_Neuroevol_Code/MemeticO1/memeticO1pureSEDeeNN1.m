...
% Basic info.
numPat = size(trainData.in,1); %number of patterns
...
if nargin < 5
    ...
    [sol0 numEval] = feval(memParams.initSolFunc,memParams,trainData); %#ok<*NCOMMA>
...
%TEST take 2 best parent from initial population 
BestParentChosen = solPostInit(1:2, 1:end);
%----------------------------------------------
...
while (totEval <= memParams.maxEval) && (dei <= memParams.maxIter) && (bestCost > memParams.minCost) && (~doStop)
    ...
    if memParams.doCrossOver
        coSols = doCrossOver(BestParentChosen,memParams);
    end
...
%    [solPostIL numEval] = doIndivLearn1(solPostGlob, memParams, trainData);
    
    % ADDING SIMULATED ANNEALING HERE 
    solPost_SA = SimulatedAnnealing(solPostGlob(1,:), solPostGlob, costFuncName, trainData, memParams.vis_verbose, memParams);
    
    % *** Trim and expand
    % Trim the x% (of NG size) solutions that are sufficiently different from each other
    solPostInit = trimSolutionsGen(solPost_SA, memParams);
    ...
    % **** Visualizations
    if memParams.vis_verbose == 2   % the function that plot out graph!
        % Cost
